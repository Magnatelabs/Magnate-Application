...
# Since we've added a custom user model to Zinnia, and there are some
# new tables involved, we also need this custom south migration.
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding M2M table for field authorized_users on 'Entry'
        m2m_table_name = db.shorten_name(u'zinnia_entry_authorized_users')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('entry', models.ForeignKey(orm['zinnia.entry'], null=False)),
            ('user', models.ForeignKey(orm[u'auth.user'], null=False))
        ))
        db.create_unique(m2m_table_name, ['entry_id', 'user_id'])


    def backwards(self, orm):
        # Removing M2M table for field authorized_users on 'Entry'
        db.delete_table(db.shorten_name(u'zinnia_entry_authorized_users'))
