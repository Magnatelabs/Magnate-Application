...
### MONKEY PATCH
###
### This file is essentially zinnia.models.entry.
### Except that all lines setting related_name for ManyToMany fields are commented out.
###
### This is because Zinnia sets related_name to a fixed value "entries", and then
### we cannot inherit two non-abstract models from the same class.
###
### Also, the last non-abstract model, Entry, is commented out to avoid clashing.
### In the future if Zinnia switches to something like related_name="entries_%(app_label)s_%(class)s", then this will solve the problem.
###
### Otherwise, you can do the same thing. Copy models/entry.py over 
### and comment out all lines with related_name set in ManyToMany relationships
### so that Django could automatically assign names to tables, and we'd avoid the name clash.
### Also, comment out the non-abstract model Entry in the end.
###
### 
### I know this is not very graceful, but this design is so much more secure.
